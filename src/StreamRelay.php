...
use KCTLD\Spiral\Goridge\Exception\HeaderException;
use KCTLD\Spiral\Goridge\Exception\InvalidArgumentException;
use KCTLD\Spiral\Goridge\Exception\RelayException;
use KCTLD\Spiral\Goridge\Exception\TransportException;
/**
...
class StreamRelay extends Relay
{
    ...
    public function waitFrame() : Frame
    {
        ...
        while ($length > 0) {
            ...
            if ($buffer === \false) {
                $message = \vsprintf('An error occurred while reading payload from the stream: %s', [$this->getLastErrorMessage()]);
    ...
    public function send(Frame $frame) : void
    {
        ...
        if (@\fwrite($this->out, $body, \strlen($body)) === \false) {
            $message = \vsprintf('An error occurred while write payload to the stream: %s', [$this->getLastErrorMessage()]);
            throw new TransportException($message);
        }
    }
    /**
    ...
    private function assertReadable($stream) : bool
    {
        ...
        return \in_array($meta['mode'], $available, \true);
    }
    /**
    ...
    {
        $meta = \stream_get_meta_data($stream);
        return !\in_array($meta['mode'], ['r', 'rb'], \true);
    }
}
