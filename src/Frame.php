...
final class Frame
{
     ...
     * @var positive-int
     */
    public const CODEC_RAW = 0x4;
    public const CODEC_JSON = 0x8;
     ...
     */
    public const BYTE10_STREAM = 0x1;
    /**#@-*/
    /**
     * @var string|null
     */
    ...
    {
        $header = \pack('CCL', self::VERSION << 4 | \count($frame->options) + 3, $frame->flags, \strlen((string) $frame->payload));
        if ($frame->options !== []) {
    ...
    {
        return [\ord($header[1]), (\ord($header[0]) & 0xf) - 3, \ord($header[2]) | \ord($header[3]) << 8 | \ord($header[4]) << 16 | \ord($header[5]) << 24];
    }
    /**
